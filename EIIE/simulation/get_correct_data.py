...
def generate_data(begin_time, end_time, tag: str = None):
    new_df = pd.DataFrame()
    for each_symbol in ['USD.JPY','USD.CAD']:
        df = pd.read_csv(f'EIIE\simulation\data\{each_symbol}-30-Min.csv')
        df['tic'] = each_symbol
    ...
    if tag == 'train':
        new_df.to_csv(
            f'EIIE\simulation\{begin_time}-{end_time}-train_data.csv')
    elif tag == 'test':
        new_df.to_csv(r'EIIE\simulation\test_data.csv')


if __name__ == '__main__':
    generate_data(begin_time='2022-01-01', end_time='2023-12-31', tag='train')
